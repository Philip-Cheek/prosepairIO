...
module.exports = function(server){
	...
	io.sockets.on('connection', function(socket){
		...
		if (clientManager.cleanConnect(socket.id)){

			console.log('we in')
			clientManager.firstConnect = false;
			...
			socket.on('repeatNickname', function(nickname){
				clientManager.checkRepeat(nickname, function(result){
					console.log('we got result', result)
					socket.emit('nicknameRepeat',result);
				});
			});
